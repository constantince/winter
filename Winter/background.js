...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.action) {
    ...
    case 'PAUSE_PROCESSING':
      console.log('⏸️ Pausing processing at index:', processingState.currentIndex);
      pauseProcessing();
      sendResponse({ status: 'paused' });
      break;

    case 'RESUME_PROCESSING':
      console.log('▶️ Resuming processing from index:', processingState.currentIndex);
      resumeProcessing();
...
async function processNextUrl() {
  if (processingState.status !== 'processing') {
    console.log('⏹️ Processing stopped, current status:', processingState.status);
    return;
  }
  ...
  console.log(`🔄 Processing URL ${processingState.currentIndex + 1}/${processingState.urls.length}:`, currentUrl);
  ...
  try {
    ...
    processingState.progress = (processingState.currentIndex / processingState.urls.length) * 100;
    
    console.log(`✅ URL processed successfully. Progress: ${processingState.progress.toFixed(2)}%`);
